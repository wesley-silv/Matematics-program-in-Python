def tabuada():
  ...
  value_multiply = float(input("Digite o valor que deseja multiplicar, podendo ser real ou inteiro: ")) # float deixa o número flutuante, assim é possível multiplicar inteiros por flutuantes.
  print('Menor número:', min(table_of_numbers)," Quantidade de elementos:",len(table_of_numbers))
  ...
  for i in table: # i indica a posição dentro da lista table
    for index in table_of_numbers: # index indica uma posição dentro da lista table_of_numbers
      ...
      print("{} X {} =".format(i, value_multiply).upper(), index)
      import time
      time.sleep(1)
    print(answer)
